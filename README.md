...
`v0.1.0-alpha.3` には、候補確定後に変換サーバーが停止し、設定画面が開かず、ローマ字がそのまま入力される不具合があります。新規インストールや更新には `v0.1.0-alpha.4` を使用してください。

`v0.1.0-alpha.4` は Windows CI で変換サーバー経由の入力・候補確定テストに成功しています。設定画面とIMEの実機操作、学習効果、CUDAでの動作は引き続き検証中です。友人への配布は実機確認後にしてください。
